...
- **FR-10.6 Retention** A retention policy bounds how many timestamped backups accumulate. Default: keep all backups (no automatic deletion), with an opt-in `--keep <n>` to prune the oldest beyond `n`. Pruning never deletes a backup created in the current run. **Implemented (PGLM-39):** `BackupOptions.keep` / CLI `--keep <n>` (positive integer); prunes the oldest `<dataDir>.bak-*` siblings, lexicographic-by-name = chronological, and force-retains the current run's backup.
