...
Each detected object becomes an `UnsupportedObject { kind, name }` entry (where `name` is the qualified object name) in the `ReconstructionReport`. The CLI prints these prominently (see CLI integration). Per **FR-9.6** they are never dropped silently. The `onUnsupported: 'warn' | 'error'` option (default `'warn'`) controls whether their presence is fatal: `'warn'` rebuilds the app-class schema anyway and lists them; `'error'` throws **before any DDL runs**, leaving the target untouched (for strict, no-surprises environments). **Implemented (PGLM-38)** — exposed as `reconstructSchema(source, target, { onUnsupported })`, on `MigrateOptions.onUnsupported`, and as the CLI `--on-unsupported <warn|error>` flag.
...
- **`src/introspect.ts`** — `introspectSchema` is the existing catalog-reading layer (tables, columns, FKs, sequences) and uses the same SQL style. Reconstruction adds the missing reads (enum labels, constraint defs, index defs, attrdef expressions) in the sibling `src/reconstruct.ts`. Both reuse the shared catalog helpers in `src/catalog.ts` — `systemSchemaFilter(alias)`, `regclassLiteral`, etc. (PGLM-42) — and the explicit-join pattern from `introspectForeignKeys` (which deliberately avoids search-path-sensitive `regclass::text`).
