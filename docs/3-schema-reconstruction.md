...
Everything the PGLM-74 audit found is fixed — multi-schema sources, sequence parameters, `OWNED BY`, and incomplete unsupported-object detection — and **custom types are now complete**: enums, domains, composites ([`16-custom-types.md`](16-custom-types.md), resolving doc 9 OQ-9.5) and ranges ([`17-range-types.md`](17-range-types.md)) are all reconstructed.
...
- **NG-3.5 (out of scope)** Full `pg_dump` parity: views, functions, triggers, RLS policies, partitioning, operator classes, comments, grants. These form a long tail that turns a focused tool into a `pg_dump` reimplementation. If a source uses them, the standalone migrator **detects and reports** them as unsupported rather than silently dropping them — `ReconstructionReport.unsupported`, folded into `MigrationReport.warnings`, and escalated to a pre-DDL throw under `onUnsupported: 'error'`. **Detection is complete** as of PGLM-79/80: views, materialized views, partitioned and foreign tables, functions, triggers, RLS policies, rules, operator classes, collations, comments, grants, and extensions. (Domains and composites left this list in PGLM-92, and ranges in PGLM-95, as each moved into scope — a reconstructed type must not also be reported as missing. The one remaining type case is a range depending on a canonical/subdiff function, which Postgres cannot create in a single statement.)
