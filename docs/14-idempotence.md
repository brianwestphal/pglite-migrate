...
- **FR-14.5 Opt-in idempotent transfer.** A caller MAY opt into an idempotent strategy via `MigrateOptions` (`onExisting: 'error' | 'truncate' | 'skip'`, default `'error'`). ~~The selected strategy is reported in the `MigrationReport` so the run is self-describing.~~ **Partial as shipped:** the option exists and is honored, but `MigrationReport` does not echo the active strategy back. `skippedTables` makes `skip` inferable; a `truncate` run is indistinguishable from a clean one. Follow-up filed.
...
- **FR-14.10 Report reflects what happened.** **Partial as shipped.** `MigrationReport.skippedTables` names the tables left untouched under `skip`, and `TableResult.method`/`fallbackReason` describe how each table was copied — but truncated-then-transferred is not distinguished from plainly transferred, and the active strategy is not surfaced (see FR-14.5). Follow-up filed.
- **NFR-14.11 Detection is cheap and version-agnostic.** The non-empty probe uses only stable catalog/SQL surface so it stays portable across PGlite majors, consistent with [`2-data-migration.md`](./2-data-migration.md) NFR-2.9. **As shipped it is not yet cheap:** `prepareTarget` calls the shared `countRows` helper (`SELECT count(*)::int`), a full scan per table, where a bounded `SELECT 1 … LIMIT 1` would answer the only question being asked. Portable but O(rows); follow-up filed.
