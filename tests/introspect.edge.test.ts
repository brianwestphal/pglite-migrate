...
describe('introspectSchema (edge cases)', () => {
  ...
  it('distinguishes GENERATED BY DEFAULT AS IDENTITY from GENERATED ALWAYS', async () => {
    // attidentity is 'd' here and 'a' above. The two are not interchangeable:
    // BY DEFAULT accepts an explicit value on insert, ALWAYS does not, and
    // reconstruction emits different DDL for each.
    await db.exec(`
    ...
    const schema = await introspectSchema(db);
    const cols = schema.tables.find((t) => t.name === 'bydefault')?.columns ?? [];

    expect(cols.find((c) => c.name === 'id')?.identity).toBe('default');
    expect(cols.find((c) => c.name === 'label')?.identity).toBeNull();
  });
