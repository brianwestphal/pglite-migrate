...
- `docs/3-schema-reconstruction.md` — standalone/DDL-reconstruction mode (overview; shipped opt-in, with known gaps listed in doc 9)
...
- `docs/18-engine-construction-options.md` — detailed spec: `OpenOptions.pgliteOptions` passthrough + `--source-database`/`--target-database` (the pre-0.4.0 `template1` case)
...
Docs 7–18 are **detailed, per-feature specs** (originally design-only for deferred capabilities; all are now implemented); docs 1–6 remain the high-level overview. Keep both in sync when implementing.
...
npm run changelog-analysis -- --next 1.1.0   # diff analysis for a technical changelog
```

`changelog-analysis` (`scripts/changelog-analysis.mjs`) is the deterministic half of the **`technical-changelog` skill**: it finds the last production `vX.Y.Z` tag, buckets the line delta by area (product vs docs vs agent scaffolding vs regenerated assets) with a product-only total, and extracts the public-surface deltas — exported names per entry point (`src/index.ts`, `src/engines.ts`), `package.json` `exports` subpaths, CLI flags, and dependency changes. It prints and writes nothing; the skill reads it, then reads the real diffs, then authors `docs/technical-changelog/<base>-<next>.md`. HEAD is the *unreleased* version, so the skill must ask for the next release number rather than read `package.json`.

Both the release-notes draft and `commit-message` are generated by [`gitgist`](https://github.com/brianwestphal/gitgist) (a dev dependency). It auto-selects an AI provider, trying the signed-in CLIs first (`claude`, then codex/antigravity/gemini/opencode), then the hosted APIs (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`), and finally on-device Apple models — so neither flow pins a provider, and both work offline with `--no-ai` (commits grouped by Conventional Commit type instead). Stage your changes, then `npm run commit-message` for a draft commit subject/body.

`gitgist.config.json` holds the repo-level gitgist config. It excludes the **generated** SVGs (`assets/demos/*`, `assets/diagram.svg`) from the diff body sent to the model — machine-generated path data and keyframe blocks that were **52% of the release-notes budget** and crowded out real code. Excluded files still appear in the changed-file list and the diffstat (gitgist tells the model their diff was withheld as generated noise), so a note can still say "demos regenerated"; the authoring sources — `scripts/demo.mjs`, `scripts/diagram.mjs` — are never excluded. Pass `--no-config` to ignore it.
...
- **Release** is tag-driven. `npm run release` (interactive, `scripts/release.sh`) bumps the version, drafts the release notes, updates `CHANGELOG.md`, runs the local gate (typecheck/lint/test/e2e/build), commits `release: v{ver}`, and pushes an annotated `v{ver}` tag. `npm run release:beta` is tag-only (no version bump/commit) and pushes `v{ver}-beta.{N}`.
- **The release notes and `CHANGELOG.md`'s `## Unreleased` section are one flow, not two** (PGLM-97). The editor is seeded with the curated `## Unreleased` body *and* a `gitgist` draft of the commits since the last tag; whatever you keep becomes the `## [version]` entry, and `## Unreleased` is then **reset to `_Nothing yet._`**. Anything deleted in the editor is gone. Before this, notes came only from gitgist and nothing ever cleared `## Unreleased`, so it silently accumulated already-shipped work across releases. `--beta` does not touch `CHANGELOG.md` at all, so a beta leaves the section intact for the eventual stable release.
