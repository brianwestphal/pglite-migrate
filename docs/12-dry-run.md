...
- **FR-12.2 Report shape parity** The dry-run result reuses the real run's `MigrationReport` shape (`src/types.ts`) so the preview and the eventual real report are directly comparable. `tables` lists the tables in FK-safe insert order; `totalRows` is the sum of planned per-table row counts; `sequencesSet` reports how many sequences *would* be realigned; `deferredTables` names the FK-cycle subset; `onExisting` echoes the re-run policy a real run would apply (`skippedTables`/`truncatedTables` are always empty — a plan writes nothing).

  > **Parity is structural, not total.** A plan's `warnings` is always empty. That is exact for FK cycles — since FR-8.7 a handled cycle emits no warning, so a real run has none either — but a real run can still warn about a per-table COPY→INSERT fallback ([`7-copy-text-transfer.md`](7-copy-text-transfer.md) FR-7.11) or an unreconstructed out-of-scope object, neither of which is knowable without touching the target. Compare `tables`, `totalRows`, `sequencesSet`, and `deferredTables`; treat `warnings` as apply-time-only.
...
1. **`planMigration(source, onProgress?): Promise<MigrationReport>`** (new export from `src/migrate.ts`) — the canonical read-only path. It does exactly the source-side work of `migrate` up to but excluding any target write: introspect the source, `topologicalSort`, compute per-table counts, compute the sequence plan and warnings, and assemble a `MigrationReport`. **As shipped it takes the source engine directly rather than the full `MigrateOptions`** — a deliberate tightening of the proposal below: not accepting a `target` at all is a stronger guarantee of NFR-12.8 than accepting one and promising not to write to it. If the deferred target cross-check (NG-12.9) is ever built it will need a second parameter, which is the moment to revisit this.
