...
- **FR-14.5 Opt-in idempotent transfer.** A caller MAY opt into an idempotent strategy via `MigrateOptions` (`onExisting: 'error' | 'truncate' | 'skip'`, default `'error'`). The selected strategy is reported in the `MigrationReport` so the run is self-describing. **Shipped (PGLM-81):** `MigrationReport.onExisting` echoes the resolved policy, including the default when the caller passed none.
- **FR-14.6 `truncate` strategy.** When selected, target tables are emptied (`TRUNCATE`) before rows are transferred, so the run converges on a clean copy of the source regardless of prior contents. **As shipped**, every table is named in one `TRUNCATE TABLE a, b, c` statement rather than truncated per-table in reverse topological order. A multi-table `TRUNCATE` defers the FK check across the whole listed set, which handles mutual references and cycles uniformly — strictly stronger than the reverse-order scheme this doc originally proposed, and it removes the need for the special case the Open Questions raise.
...
- **FR-14.10 Report reflects what happened.** **Shipped (PGLM-81).** `MigrationReport` distinguishes all three outcomes: `skippedTables` (left untouched under `skip`), `truncatedTables` (held rows and were emptied under `truncate`), and everything else in `tables` as plainly transferred — with `TableResult.method`/`fallbackReason` describing *how* each was copied. `truncatedTables` is the only record that a destructive re-run discarded data; without it a `truncate` run was indistinguishable from a clean first run.
- **NFR-14.11 Detection is cheap and version-agnostic.** The non-empty probe uses only stable catalog/SQL surface so it stays portable across PGlite majors, consistent with [`2-data-migration.md`](./2-data-migration.md) NFR-2.9. **Shipped (PGLM-82):** `prepareTarget` uses `hasRows` (`SELECT 1 … LIMIT 1`) from `src/catalog.ts`, which stops at the first row. `countRows` stays for `planMigration` and `validateMigration`, which need exact counts. Making the probe cheap is also what let it run unconditionally on the `truncate` path, which is where `truncatedTables` (FR-14.10) gets its data.
