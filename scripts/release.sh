...
# Interactive release flow for pglite-migrate. Adapted from kerf / Hot Sheet.
#
# Usage:
...
ask_multiline() {
  ...
  local tmpfile
  tmpfile=$(mktemp "${TMPDIR:-/tmp}/pglite-migrate-release-notes.XXXXXX")
...
step_local_checks() {
  ...
  npm test
  echo ""
  info "Cross-major e2e (PG17 -> PG18)..."
  npm run test:e2e
  echo ""
  info "Build..."
  npm run build
...
step_git_commit() {
  ...
  # commit (e.g. recovery after a failed run), there's nothing left to stage.
  # Skip rather than fail under `set -e` so the tag-and-push step still runs.
...
step_stable_tag_and_push() {
  ...
  echo -e "    1. Run tests / e2e / lint / typecheck / build"
  echo -e "    2. Create a GitHub Release for ${tag}"
  echo -e "    3. Publish to npm with --provenance"
  echo ""
  echo -e "  ${DIM}Monitor:${RESET} https://github.com/brianwestphal/pglite-migrate/actions"
}
...
step_beta_tag_and_push() {
  ...
  echo -e "  ${DIM}Users get it via:${RESET}  npm install pglite-migrate@beta"
  echo ""
  echo -e "  ${DIM}Monitor:${RESET} https://github.com/brianwestphal/pglite-migrate/actions"
}
...
main() {
  ...
  if [[ "$BETA_MODE" == "true" ]]; then
    echo -e "${BOLD}  pglite-migrate Beta Release${RESET}"
    echo -e "  ${DIM}--beta mode: tag-only, no version-file bump.${RESET}"
  else
    echo -e "${BOLD}  pglite-migrate Release${RESET}"
  fi
  echo ""
