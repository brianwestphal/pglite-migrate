...
- **NG-4.11** The CLI prints validation as a single `Validation (<level>): OK|FAILED.` line plus a warning naming the failing tables. Doc 13's richer surface — a per-table `users: 1234 = 1234 ✓` summary and an explicit mismatch list — is not implemented.
